(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('initialSpringBootApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-initialSpringBootApp-error');
                var entityKey = httpResponse.headers('X-initialSpringBootApp-params');
                if (errorHeader) {
